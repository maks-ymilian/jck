# JCK
JCK is a car rental website

![JCK home page](home-page.png)

## Features
- Find cars in your location
- Pay with Stripe
- Post your cars for rent on the site
- Reviews
- Average rating based on reviews
- Keep track of your bookings and listings

## About
This is an ASP.NET web app that uses C# for the backend and plain HTML/CSS/JS for the frontend.

It uses
- Clerk for user account handling
- Supabase for Postgres database hosting
- Stripe for payments
- AWS S3 for image storage
- Render for hosting the frontend/backend
- GitHub Actions for auto-deploying to Render on every push
- Jest for frontend testing and xUnit for backend testing
